function [X_prime ] = Flusso_reagente( x,X )
...
kb=kf/kc;
W_dot=rho*(1+alpha)^2*(kf*(1-alpha)/(1+alpha)*p/(R*T)-4*kb*alpha^2*p^2/((1+alpha)^2*(R*T)^2));

%Sistema di equazioni differenziali
p_prime=(((2*(alpha+1)/(3*alpha+7))*(3/4*R*T/M_N+dh_N-dh_N2)-R*T/M_N2)*W_dot/u)/(2*(alpha+1)/(3*alpha+7)+p/(rho*u^2)-1);
rho_prime=p_prime/u^2;
